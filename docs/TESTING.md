...
| R-5 | ~~The Linux WWID a LUN's `vpd_unit_sn` becomes~~ **ANSWERED and confirmed on two independent samples.** `WWID = "3" + "6001405" + uuid(- becomes d), first 25 chars`, verified against `scsi_id` on an attached LUN | Decides how a node identifies its device. Still a cross-check only: the kernel's own answer is what the plugin acts on |
...
| R-11 | ~~`mapping_index` ceiling per target, and whether it is reused~~ **ANSWERED: it IS reused.** A freed index goes to the next LUN mapped | This is the "wrote to the wrong disk" fault, reachable by detaching one disk and attaching another. It is why device identity comes from the kernel's WWID and never from a path. **The ceiling is now measured too: 200 LUNs were mapped to one target with no refusal and `mapping_index` ran 1 to 200 unbroken.** 201 to 245 is untested and cannot be reached cheaply — that model holds 256 LUNs and the array used for it already had 11, so filling the rest would leave a production NAS at its ceiling for the duration |
...
| R-28 | ~~Which characters DSM accepts in a LUN name~~ **ANSWERED by asking DSM: 33 characters, one create each, verified by a lookup and deleted afterwards.** Accepted: letters, digits, `-`, `.`, `:` — and `\`, which this plugin refuses anyway. Everything else is refused with **18990503**: `_` `~` `!` `#` `$` `%` `^` `&` `*` `(` `)` `+` `=` `@` `,` `;` `?` `/` `\|` `[` `]` `{` `}` `<` `>` `` ` `` `'` `"` and space | The register previously named four of them, which was a sample rather than the set. It matters because **Proxmox VE puts a snapshot name inside a volume name** when a snapshot includes RAM, and a `pve-configid` is `[a-z][a-z0-9_-]+` — so both `_` and `-` are ordinary there. The plugin now whitelists `[A-Za-z0-9.:-]` and maps `_` to `.` reversibly |
