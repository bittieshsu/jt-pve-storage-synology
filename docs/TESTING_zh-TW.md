...
它刻意寫得誠實。無法確認卻仍然推測的 storage plugin，比明確拒絕的更糟，所以每一項儲存伺服器端的事實都標明來源，任何無法確認的事情也都直接寫出來。
...
- **`restored_time` 存在**，這是「從快照還原 LUN」是一個真實且會被記錄的操作的證據——見未確認驗證紀錄 R-1。
...
3. multipath `conf.d` drop-in 需要的兩個字串是 `vendor "SYNOLOGY"` 與 `product "Storage"`。**兩個都不可能從 NAS 的 API 取得**。它們是 SCSI INQUIRY 的回應。猜錯會讓 drop-in 悄悄失效，而 `no_path_retry` 不生效就代表所有路徑消失時會無限排隊。
...
| 比快照時間點更新的快照會活著嗎？| **會**。還原到三個之中最舊的那一個，三個都還在 |
| 有被記錄嗎？| `restored_time` 從 0 變成還原當下的 epoch 秒 |

第二個答案值得停下來看，因為相關專案必須**拒絕**越過較新快照的倒回，在那些儲存伺服器上較新的快照會被銷毀，而在不告知的情況下讓 PVE 執行那個操作的 plugin，等於刪掉使用者還看得到的快照。這裡什麼都不會被銷毀，所以 `volume_rollback_is_possible` 不需要那道限制，一個 storage 也可以反覆倒回。

### 快照與複本之間沒有依賴鏈

這個家族裡其他每一台儲存伺服器，都會拒絕刪除「有別的東西依賴它」的物件，而那兩種拒絕都造成過真實的缺陷。Synology 兩種都不拒絕：
...
兩份公開的參考實作都只用 `/dev/disk/by-path` 認裝置。**在這台儲存伺服器上那不安全**。這就是為什麼上面那條 WWID 推導是承重的，不是方便而已：一個裝置只有在**核心自己的識別**與被要求的那顆 LUN 相符時才會被接受。
...
- **十六個同時發出的建立全部成功**，耗時 15 秒，而儲存伺服器的內容與 API 回報的一致，沒有遺失也沒有重複的 LUN。每個建立約 1 秒，看起來 DSM 內部是序列化的，這也解釋了 Cinder 為什麼把每個請求都包在行程級鎖裡；但這裡沒有任何東西為了正確性需要那個鎖。
...
- 對 `SYNO.Core.User` `create` 傳 `expired=now`，會產生一個存在但無法登入的帳號。症狀是一個看起來像權限問題的 402：一個猜出來的參數建立了一個壞掉的帳號，而且沒有任何跡象。
...
**底線這件事很重要**。Proxmox VE 的 storage id 可以含底線，而相關專案會把 storage id 折進儲存伺服器的物件的名稱裡，所以在這裡折疊不能用 `_`，含底線的 storage id 必須被轉成合法字元。而那會把相關專案踩過的撞名問題帶回來：兩個不同的 storage id 折成同一個前置字串，然後彼此都看得到、也刪得掉對方的磁碟。所以「拒絕第二個這樣的 storage」那道檢查，在這台儲存伺服器上不是可選的。
...
這比這個家族裡其他任何一台儲存伺服器都單純——完全不需要對齊運算。但後半段要注意：知識中心記載**最小 LUN 容量 1 GB**，而 **API 並不強制**。一顆 1 位元組的 LUN 被毫無異議地建立了。所以那個最小值是 plugin 自己要守的，而一個接受極小磁碟的 storage 等於在依賴沒有文件的行為。
...
現在 `alloc_image` 只取一次，然後傳給三者。`find_free_diskname` 接受一個選擇性的清單參數；其他所有呼叫者的行為和基底類別完全一樣。而 `wait_unlocked` 原本用 `sleep 1` 輪詢，驗證紀錄記錄的鎖清除時間是 **0.20 秒**，所以五分之一秒要花掉整整一秒，每一次建立、每一次快照，而且都在鎖裡面。現在它以 0.2 秒輪詢。
...
**R-25 被記成已量測，而它從來不是**。快照時間戳那段註解說 `create_time` 已經「對 NAS 自己的時鐘確認過」。這份驗證紀錄裡沒有任何這樣的量測。而它也無法用唯讀方式確定：**LUN 根本沒有 `create_time` 這個欄位**，所以必須真的拍一個快照再讀回來。
...
這是這個家族目前為止這個錯誤最糟的形式。`Naming` 被位移的呼叫拒絕了它本該允許的東西；`Multipath` 的則是忽略了一個設定；而這一個**把一個安全問題答錯了，而且錯得毫無跡象，方向又是往「採取動作」那一邊。**
...
| # | 問題 | 為什麼重要 |
|---|---|---|
| R-1 | ~~方法名稱、它的參數，以及倒回是否安全~~ **完整解答：`restore_snapshot(src_lun_uuid, snapshot_uuid)`**。LUN uuid 不變、比快照時間點更新的快照存活、`restored_time` 會記錄 | 所以倒回可以出貨，而且 `volume_rollback_is_possible` **不需要**相關專案那道拒絕。只送快照會被 18990508 拒絕 |
...
| R-9 | ~~那些清單有沒有伺服器端上限~~ **依清單分別解答，在 DSM 7.4.1 上以唯讀方式重新量測**。`LUN list` 與 `Target list` **完全忽略 `offset`／`limit`**（`limit=1` 回來的和不帶參數回來的是同一份完整清單）而且兩者都不回報總數。`LUN list_snapshot` 是例外：它**會**在JSON 陣列旁邊回報一個 `count`，而那個數字在全部九顆 LUN 上都與列數一致，包括其中一顆真的有快照的。量測時是 9 顆 LUN、4 個 target、1 張快照；從這次量測無法排除在更高的數量上存在門檻 | **被靜默截斷的清單讀起來就是「全部就這些」**，而讀它的程式碼決定什麼可以刪。參數被忽略是最強的證據：會分頁的 API 一定會遵守 `limit`。而在**有**回報總數的地方，現在會去對照：`assert_room_for_snapshot` 在 `count` 與列數不一致時拒絕，因為它數的是JSON 陣列，而在那裡少算會往「把快照拍下去」的方向失敗 |
...
| R-11 | ~~每個 target 的 `mapping_index` 上限，以及會不會重用~~ **已解答：會重用**。被釋放的編號會給下一顆被對應的 LUN | 這就是「寫到別人的磁碟」那種錯誤，而卸離一顆磁碟再掛上另一顆就到得了。這也是為什麼裝置身分來自核心的 WWID，絕不來自路徑。**上限本身現在也量測過了：200 顆 LUN 對應到同一個 target，DSM 沒有拒絕，`mapping_index` 從 1 到 200 連續。** 201 到 245 沒有測，而且不容易便宜地測到：那個機型放得下 256 顆 LUN，而用來測的那台已經有 11 顆，把剩下的填滿會讓一台正式 NAS 在測試期間停在上限 |
| R-12 | ~~DSM 能不能承受並行請求~~ **已解答：十六個同時發出的建立全部成功**，而儲存伺服器的內容與 API 回報的一致。每個約 1 秒，看起來 DSM 內部是序列化的 | Cinder 把每一個請求都包在行程級的鎖裡；這裡沒有任何東西為了正確性需要它 |
...
| R-28 | ~~DSM 的 LUN 名稱接受哪些字元~~ **已解答，而且是問 DSM 得到的：33 個字元，每個各建立一次、以查詢核對、事後刪除**。接受：字母、數字、`-`、`.`、`:`，以及 `\`（而這個 plugin 仍然不用它）。其餘全部以 **18990503** 拒絕：`_` `~` `!` `#` `$` `%` `^` `&` `*` `(` `)` `+` `=` `@` `,` `;` `?` `/` `\|` `[` `]` `{` `}` `<` `>` `` ` `` `'` `"` 與空白 | 驗證紀錄先前只列出其中四個，那是樣本不是全集。這件事重要，是因為**含記憶體的快照會把快照名稱放進磁碟名稱裡**，而 `pve-configid` 是 `[a-z][a-z0-9_-]+`——所以 `_` 和 `-` 在那裡都是尋常字元。Plugin 現在改成白名單 `[A-Za-z0-9.:-]`，並把 `_` 可逆地對應到 `.` |
...
### F——清理，而且絕不能把執行中 guest 的磁碟抽走

只有指令列：這一組沒有任何一項可以從網頁介面做到。網頁介面驗證的是它的後果——做完之後，那個 storage 上的 guest 還是開得起來。
...
讀程式碼的答案是「會」：`_api` 每次呼叫都回傳新物件，而且沒有任何地方留著它，所以 client 在 plugin 方法回傳時就被釋放。讀不等於量測，所以去量測了。暫時在 `DESTROY` 裡加一行，印出它的 pid、`PVE::RESTEnvironment->is_worker` 是不是真、以及 `${^GLOBAL_PHASE}`;然後用 API 權杖從 **HTTPS API** 驅動一次擴充，那是一個在 `perl -T` 底下的 pvedaemon worker,既不是 `qm` 也不是 `pvesh`。worker 自己的工作記錄：
...
| | 砍在哪裡 | 結果 |
|---|---|---|
| **H1** | 快照，分別在 0.3 秒、0.8 秒、1.3 秒 | 要嘛兩邊都沒有，要嘛**兩邊都有**：設定裡有，儲存伺服器上也有，而且帶著 `taken_by`。從來沒有出現只有一邊有的情況。事後再用同一個名字會被當成重複而拒絕，那是對的 |
| **H2** | 擴充，分別在 0.3 秒、0.8 秒、1.3 秒、2.0 秒 | 要嘛都沒變，要嘛兩邊都是新的容量。沒有一次對不上。萬一真的對不上，`qm rescan --vmid` 是校正的方法 |
| **H3** | 完整複製，在 3 秒 | 會留下帶著 `lock: clone` 的 `<新id>.conf`，而**儲存伺服器上沒有留下磁碟**。`qm destroy` 會拒絕並說 *VM is locked (clone)*；`qm unlock <新id>` 之後再 `qm destroy <新id>` 就清乾淨，逐步驗證過，兩邊都沒有殘留 |
| **H4** | 離線遷移，在 1.0 秒 | 設定留在來源節點，guest 在那裡起得來也跑得動，目標節點什麼都沒有 |

事後在三個節點上：唯一的 multipath 對應與追蹤記錄，屬於那台還在執行的 guest，而每個節點的 `pve-syno-reap --all` 都回報沒有殘留。儲存伺服器回到 0 個 LUN、0 個快照。

**方法錯了兩次才對，而那才是值得留下來的部分**。在有重導向的情況下，`cmd & p=$!` 拿到的是 bash 分出來的子 shell 的 pid，不是它接著 exec 的那個命令，所以前兩輪砍掉的是包裝殼，而操作變成孤兒跑到完成。更糟的是，在那種假的中斷之後兩秒去取樣兩邊，取到的是還在收尾的操作，於是產生一個乾淨、可重現、而且完全錯誤的結果：十次跑出來「顯示」被中斷的擴充會讓儲存伺服器比 VM 設定多一級。事實不是這樣。做法是改成 `( exec cmd ) &`，而且在送出訊號之前，先用 `ps -o args= -p $!` 確認那個 pid 真的就是那個命令。
...
第二件說明了相關專案為什麼**拒絕**越過較新的快照倒回：在那些儲存伺服器上較新的快照會被銷毀。這裡什麼都不會被銷毀，所以那道限制不需要，一顆磁碟也可以反覆倒回。
...
在這台儲存伺服器上「連結」這個字說得太保守：DSM 的 `clone_from_snapshot` 做的是 **reflink**，所以新的 LUN 是獨立的（之後刪掉來源快照也不影響它）而建立當下不佔空間。已量測：在複本**執行中**把範本的 LUN 刪除，複本照樣跑。
...
| mapping_index 會重用——被釋放的編號會給下一顆被對應的 LUN | 殘留的裝置路徑會指向另一顆磁碟。兩份公開用戶端都只用路徑認裝置；在這台儲存伺服器上那不安全，這也讓 WWID 推導成為承重結構 |
